...
DAMPING = 0.7
SUBSTEPS = 50
FLUID_DENSITY = 0.9  # Résistance globale de l'eau
...
class Node:
    def __init__(self, x, y, is_muscle=False, amplitude=0.3, phase=0.0, period=120, nodes_ref=None, pause_max=False, pause_min=False, threshold=0.5, coef_pause = 0):
        ...
        if is_muscle and nodes_ref and len(nodes_ref) == 2:
            ...
            self.base_angle = angle2 - angle1 # en rad
...
# 1. Mise à jour des cibles de muscles
        for n in self.nodes:
            if n.is_muscle:
...
# 4. Physique de l'eau (loi 1: 3 ème loi de Newton: on regarde chaque membre individuellement, pour voir comme ce mouvement déplace l’eau en supposant que l’eau est immobile:
#  si le membre pousse l’eau dans une direction, on doit lui appliquer une force dans l’autre sens) ==> proportionnel au carré de la vitesse
        for e in self.edges:
            n1, n2 = self.nodes[e.n_a], self.nodes[e.n_b]
        ...
        for n in self.nodes:
            if n.is_muscle and n.nodes_ref:
                ...
                if v1.length() > 0 and v2.length() > 0:
                    ...
                    # 2. Facteur d'efficacité : Max à 0 rad, décroît vers 0 à 180° (pi rad), courbe en cosinus pour une transition douce
...
def spawn_random_creature(num_nodes=6):
    ...
    for i, n in enumerate(nodes):
        ...
        if connection_count[i] >= 2:
            # n.is_muscle = random.choice([True, False]) # si on veut avoir 1 chance sur 2 d'être un muscle. 
            n.is_muscle = True # Pour l instant tout les nodes ayant 2 edges sont considéré comme des muscle. Ca peut être bien si on fixe la possibilité d'avoir une amplitude de 0 par ex
            if n.is_muscle:
                n.amplitude = random.uniform(0, math.pi/4) # 45 degrés max, 
                n.phase = random.uniform(0, 2 * math.pi)
                n.period = random.randint(FPS*2, FPS * 5)
