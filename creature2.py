...
class Muscle:
    ...
    def apply_physics(self, nodes, timer):
        ...
        cycle = (timer % params.GEN_DURATION) / params.GEN_DURATION
        angle_target = math.sin(2 * math.pi * cycle * self.freq + self.phase) * self.amplitude
        ...
        rigidity = 0.9
        strength = 0.5 * angle_error
        fx = -dy / dist * strength
        fy = dx / dist * strength
        node_b.apply_force(fx, fy)
        node_a.apply_force(-fx, -fy)
        length_error = dist - self.base_length
        rigidity = 0.5
        fx_len = dx / dist * length_error * rigidity
        fy_len = dy / dist * length_error * rigidity
        node_a.apply_force(fx_len, fy_len)
        node_b.apply_force(-fx_len, -fy_len)
        self.energy_spent += abs(fx) + abs(fy) + abs(fx_len) + abs(fy_len)
