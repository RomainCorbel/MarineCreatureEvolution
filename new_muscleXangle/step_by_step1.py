...
class Node:
    def __init__(self, x, y, is_muscle=False, amplitude=0.3, phase=0.0, period=120, nodes_ref=None):
        self.x, self.y = x, y
...
class Creature:
    ...
    def update(self, frame_count):
        ...
        for n in self.nodes:
            if n.is_muscle:
                # Calcul de l'angle cible oscillant
                t = math.sin(2 * math.pi * frame_count / n.period + n.phase)
...
def spawn_pince():
    ...
    # Le pivot est au centre (0, 0)
    # pivot = Node(0, 0, is_muscle=True, amplitude=2, period=240)
    # node_gauche = Node(0, 1)
    # node_droit = Node(1,0)
    pivot = Node(0, 0, is_muscle=True, amplitude=0.5, period=120)
    node_gauche = Node(0, 1)
    node_droit = Node(0,-1)
    # 2. Lien de parenté pour le muscle
    # Le pivot doit savoir quels points il influence pour l'angle
    pivot.nodes_ref = (node_gauche, node_droit)
    
    # 3. Calcul de l'angle initial (base_angle)
    # On réutilise la logique de calcul d'angle entre les deux segments
    ...
    # 4. Création des Edges (les OS)
    # On relie le pivot aux deux extrémités
    edges = [
        Edge(0, 1), # Os vers la gauche (index 0 et 1)
        Edge(0, 2)  # Os vers la droite (index 0 et 2)
    ]
    
    # On retourne la créature avec la liste ordonnée des nodes
